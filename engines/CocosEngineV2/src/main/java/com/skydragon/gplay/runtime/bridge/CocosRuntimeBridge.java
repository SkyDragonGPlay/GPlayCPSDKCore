...
public final class CocosRuntimeBridge implements IEngineRuntimeBridge {
    ...
    private static final String VERSION = "2.2.6.10";
    ...
    public void removeStatusChangedListener(OnRuntimeStatusChangedListener listener) {
        if (mStatusChangedListeners.contains(listener)) {
            mStatusChangedListeners.remove(listener);
        } else {
            Log.e(TAG, "listener wasn't added!");
        }
    }
    ...
    public void notifyOnLoadSharedLibrary() {
        try {
            ...
            // Initialize native c++ codes
            Cocos2dxHelper.init(mActivity, Cocos2dxActivity.getCocosActivity());
